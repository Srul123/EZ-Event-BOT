...
} from "../../domain/campaigns/guest-session.service.js";
...
export async function startHandler(ctx: SessionContext): Promise<void> {
  ...
  if (!ctx.message || !("text" in ctx.message)) {
    ctx.reply(
      "Welcome! This is the EZ-Event bot. Use /help to see available commands.",
    );
    return;
  }
  ...
  if (token) {
    try {
      ...
      if (guestData) {
        ...
        logger.warn(
          { token, userId: ctx.from?.id },
          "Invalid or expired invite token",
        );
        ctx.reply(
          "Sorry, this invite link is invalid or has expired. Please contact the event organizer.",
        );
        return;
      }
    } catch (error) {
      logger.error(
        { error, token },
        "Error processing start command with token",
      );
      ctx.reply(
        "An error occurred while processing your invite. Please try again later.",
      );
      return;
    }
  }
  ...
  if (ctx.session.guest) {
    ...
    ctx.reply(
      `Welcome back ${ctx.session.guest.name}! 👋\n\nUse /help to see available commands.`,
    );
  } else {
