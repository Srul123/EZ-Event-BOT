 ...
 */
function normalizeHebrewText(text: string): {
  normalized: string;
  tokens: string[];
} {
  // Lower-case and trim
 ...
 */
export function extractHeadcount(
  text: string,
  allowFuzzy: boolean = false,
): HeadcountExtraction {
  ...
  for (const term of hebrewFamilyTerms) {
    if (normalizedHebrew.includes(term) || normalized.includes(term)) {
      ...
      const hasHebrewNumber = extractNumberFromHebrewWords(
        normalizedHebrew,
        false,
      ); // No fuzzy for this check
  ...
  const hebrewNumberResult = extractNumberFromHebrewWords(
    normalizedHebrew,
    allowFuzzy,
  );
  if (
    hebrewNumberResult !== null &&
    hebrewNumberResult.number > 0 &&
    hebrewNumberResult.number <= 20
  ) {
  ...
  if (
    normalized.match(/\bjust\s+me\b/i) &&
  ...
  const hebrewSingularChild =
    /(?:אני\s+(?:ו|ועם)\s*)?(?:הילד|ילד)\s*(?:אחד|אחת)?(?!ים)/i;
  ...
  const englishSingularChild =
    /\b(?:me\s+(?:and|with)\s+)?(?:my\s+)?child\b(?!ren)/i;
  if (
    englishSingularChild.test(normalized) &&
    !normalized.includes("children")
  ) {
    // Check if there's already a count mentioned
    const existingCount = normalized.match(/\b(\d+)\b/);
...
export function interpretWithRules(text: string): Interpretation {
  ...
  // Check for MAYBE
  const hasMaybe = MAYBE_KEYWORDS.some((keyword) =>
    normalized.includes(keyword.toLowerCase()),
  );
  if (hasMaybe) {
    return {
