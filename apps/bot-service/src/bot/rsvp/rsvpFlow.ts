...
function detectChangeIntent(
  ...
  const hasChangeKeyword = CHANGE_KEYWORDS.some((keyword) =>
  ...
  const hasCorrectionKeyword = CORRECTION_KEYWORDS.some((keyword) =>
    normalizedText.includes(keyword),
  );
...
export async function handleIncomingTextMessage({
  ...
  if (
    ...
    interpretation.headcountExtraction.headcount !==
      flowContext.currentHeadcount &&
    (interpretation.rsvp === "UNKNOWN" ||
      (interpretation.rsvp === "NO" && hasCorrectionKeyword))
  ) {
