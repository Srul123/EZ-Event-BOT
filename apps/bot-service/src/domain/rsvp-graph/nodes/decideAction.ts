...
function detectChangeIntent(
  ...
  const hasCorrectionKeyword = CORRECTION_KEYWORDS.some((kw) =>
  ...
  if (guestContext.currentRsvpStatus === "YES") {
    if (
      ...
      interpretation.headcountExtraction.headcount !==
        guestContext.currentHeadcount
    ) {
      return true;
    }
    ...
    if (interpretation.headcountExtraction.kind === "exact") {
      ...
      const hasRsvpKeywords = rsvpKeywords.some((kw) =>
        normalized.includes(kw),
      );
      if (
        (!hasRsvpKeywords && interpretation.rsvp === "UNKNOWN") ||
...
export function decideAction(input: DecideActionInput): Action {
  const { guestContext, interpretation, headcountExtraction, messageText } =
    input;
  ...
  if (
    (guestContext.currentRsvpStatus === "YES" ||
      guestContext.currentRsvpStatus === "NO") &&
